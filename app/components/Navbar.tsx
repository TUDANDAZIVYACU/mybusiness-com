...
import { checkAndAddBusinessbi } from '../actions'
...
const Navbar = () => {
   ...
   const navLinks = [
        ...
        { href: "/give", label: "Vendre", icon: HandHeart },
        { href: "/transactions", label: "Transactions", icon: Receipt },
        { href: "/products", label: "Stock Produits", icon: ShoppingBasket }
        
    ]

    useEffect(() => {
        if (user?.primaryEmailAddress?.emailAddress && user.fullName) {
            checkAndAddBusinessbi(user?.primaryEmailAddress?.emailAddress, user.fullName)
        }
    }, [user])
